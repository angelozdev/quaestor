...
def _transfer(world: World, destination: str) -> Transaction:
    ...
    if tx_id is None:
        raise AssertionError(f"no proposed contribution into {destination!r} in this scenario")
...
def _occurrence(world: World, payee: str) -> RecurringOccurrence:
    ...
    occ = world.session.exec(select(RecurringOccurrence).where(RecurringOccurrence.recurring_id == item_id)).first()
    if occ is None:
...
def given_planned_income(world: World, amount: str, currency: str, account: str, due: str, **_) -> None:
    tx = Transaction(
...
def given_monthly_obligation(world: World, amount: str, currency: str, payee: str, account: str) -> None:
    _materialize_manual_recurring(world, amount, currency, payee, account, TxType.expense)
...
def given_monthly_income(world: World, amount: str, currency: str, payer: str, account: str) -> None:
    _materialize_manual_recurring(world, amount, currency, payer, account, TxType.income)


@step(r'"(?P<account>[^"]+)" is the account transfers come from')
def given_default_source_account(world: World, account: str) -> None:
    settings_svc.update_settings(world.session, default_source_account_id=_account_id(world, account))
...
def given_proposed_contribution(world: World, amount: str, currency: str, account: str) -> None:
    tx = Transaction(
        date=world.today,
...
def when_view_outstanding_window(world: World, span: str, offset: str, direction: str) -> None:
...
def when_confirm_payment(
    ...
    world.attempt(
        ...
        date=(world.today - timedelta(days=int(back)) if back is not None else None),
    )
...
def when_assistant_confirms_payment(world: World, payee: str) -> None:
    ...
    def action():
        ...
        return temporal.confirm_payment(world.session, temporal.ConfirmPaymentInput(tx_id=tx.id))
...
def when_assistant_asked_outstanding(world: World, days: str) -> None:
    def action():
        ...
        world.assistant_answer = temporal.to_pay(
            world.session,
            temporal.ToPayInput(since=world.today, until=world.today + timedelta(days=int(days))),
        )
...
def then_list_shows_in_bucket(world: World, payee: str, bucket: str, days: str | None = None) -> None:
    ...
    assert payee not in [t.payee for t in other], f"{payee!r} appears in both groups; they must be mutually exclusive"
...
def then_list_shows(world: World, payee: str) -> None:
    queue = _viewed_queue(world)
    assert payee in _payees(queue), f"{payee!r} is not in the outstanding list ({_payees(queue)})"
...
def then_list_order(world: World, upper: str, lower: str) -> None:
    ...
    assert upper in payees and lower in payees, f"expected both {upper!r} and {lower!r} in the list, got {payees}"
...
def then_list_shows_contribution(world: World) -> None:
    ...
    assert TxType.transfer in kinds, f"no proposed contribution in the outstanding list (kinds={kinds})"
...
def then_list_omits(world: World, payee: str, days: str | None = None) -> None:
    queue = _compute_queue(world, int(days)) if days else _viewed_queue(world)
    assert payee not in _payees(queue), f"{payee!r} is still in the outstanding list ({_payees(queue)})"


@step(r"the outstanding list(?: for the next (?P<days>\d+) days)? is empty")
def then_list_empty(world: World, days: str | None = None) -> None:
    queue = _compute_queue(world, int(days)) if days else _viewed_queue(world)
    assert queue.is_empty, f"the outstanding list is not empty: {_payees(queue)}"


@step(r"the outstanding list has no overdue group")
def then_no_overdue_group(world: World) -> None:
    queue = _viewed_queue(world)
    assert not queue.overdue, f"an overdue group was rendered: {[t.payee for t in queue.overdue]}"
...
def then_list_item_count(world: World, count: str) -> None:
    ...
    assert len(payees) == int(count), f"expected {count} item(s) outstanding, got {len(payees)}: {payees}"
...
def then_outstanding_total(world: World, amount: str, currency: str, days: str | None = None) -> None:
    if days:
        total = _compute_queue(world, int(days)).total_cop_cents(fx.get_trm(world.session))
    else:
        world.require_clean("viewing the outstanding list should have succeeded")
        total = world.queue_total
    assert total is not None, "the outstanding list was never viewed"
    expected = major_to_cents(amount)
    assert total == expected, f"expected a total of {amount} {currency}, got {cents_to_major(total)}"
...
def then_list_not_shown(world: World) -> None:
    ...
    assert world.queue is None, (
        "the outstanding list was shown although no TRM is set (AC-20: reads must fail loud, never assume a rate)"
    )
    world.consume_rejection((MissingRate,), "viewing the outstanding list without a TRM")
...
def then_payment_waiting(world: World, payee: str) -> None:
    ...
    tx = _payment(world, payee)
    assert tx.status == TxStatus.planned, f"the payment to {payee!r} is {tx.status.value}, not waiting"
...
def then_payment_not_waiting(world: World, payee: str) -> None:
    ...
    tx = _payment(world, payee)
    assert tx.status != TxStatus.planned, f"the payment to {payee!r} is still waiting"
...
def then_payment_spent_for(world: World, payee: str, amount: str, currency: str) -> None:
    world.require_clean(f"confirming the payment to {payee!r}")
    tx = _payment(world, payee)
    assert tx.status == TxStatus.posted, f"the payment to {payee!r} is {tx.status.value}, not spent"
    expected = major_to_cents(amount)
    assert tx.amount == expected and tx.currency == currency, (
        f"expected {amount} {currency}, got {cents_to_major(tx.amount)} {tx.currency}"
    )
...
def then_payment_unchanged(world: World, payee: str, amount: str, currency: str) -> None:
    tx = _payment(world, payee)
    expected = major_to_cents(amount)
    assert tx.amount == expected and tx.currency == currency, (
        f"expected {amount} {currency}, got {cents_to_major(tx.amount)} {tx.currency}"
...
def then_planned_details(world: World, payee: str, category: str, notes: str) -> None:
    world.require_clean(f"planning the payment to {payee!r}")
    tx = _payment(world, payee)
...
def then_restored_payment_intact(world: World, payee: str, amount: str, currency: str, days: str) -> None:
...
def then_nothing_spent(world: World, payee: str) -> None:
    ...
    posted = [
        t
        for t in world.session.exec(select(Transaction).where(Transaction.payee == payee)).all()
...
def then_occurrence_state(world: World, payee: str, state: str) -> None:
    ...
    assert occ.status == expected, f"this month's turn of {payee!r} is {occ.status.value}, expected {expected.value}"
...
def then_obligation_active(world: World, payee: str) -> None:
    ...
    assert item is not None and item.active, f"the obligation to {payee!r} was deactivated"
...
def then_transfer_pair(world: World, account: str) -> None:
    ...
    legs = transactions.list_transactions(world.session, status="posted", type=TxType.transfer)
...
def then_contribution_still_waiting(world: World, account: str) -> None:
    tx = _transfer(world, account)
    world.session.refresh(tx)
    assert tx.status == TxStatus.planned, f"the contribution into {account!r} is {tx.status.value}, not waiting"
...
def then_inverted_period_rejected(world: World) -> None:
    ...
    assert "inverted" in str(err).lower(), f"the answer does not say the period is impossible: {err}"
...
def then_report_nothing_pending(world: World) -> None:
    ...
    assert world.report.pending == [], f"the retrospective still carries pending items: {world.report.pending}"
...
def then_assistant_answer_bucket(world: World, payee: str, bucket: str) -> None:
    ...
    assert payee in section, f"{payee!r} is not under the {heading} section of the assistant's answer:\n{answer}"
...
def then_assistant_answer_total(world: World, amount: str, currency: str) -> None:
    ...
    assert expected in answer, f"the assistant's answer does not report a total of {expected}:\n{answer}"


@step(r"the assistant's answer says nothing is outstanding")
def then_assistant_answer_empty(world: World) -> None:
    answer = _assistant_answer(world)
    assert "nothing outstanding" in answer.lower(), f"the assistant's answer does not say the queue is empty:\n{answer}"
