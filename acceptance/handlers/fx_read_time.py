...
def _transfer_cross_currency(session, from_id, to_id, sent_cents, received_cents, date):
    ...
    return transactions.transfer(
        session, from_id, to_id, sent_cents, None, date,
...
def given_account_with_balance(
    world: World, name: str, currency: str, amount: str, currency2: str
) -> None:
    acc = accounts.create_account(
        world.session, name, "debit", currency, balance=major_to_cents(amount)
    )
...
def _default_account_id(world: World, currency: str) -> int:
    if currency not in world.default_accounts:
        acc = accounts.create_account(
            world.session, f"{currency} Wallet", "debit", currency, balance=0
        )
        world.default_accounts[currency] = acc.id
    return world.default_accounts[currency]


def _record_default_expense(
    world: World, amount: str, currency: str, on_date, category_id: int | None = None
) -> None:
...
def given_recorded_expense_previous_month(
    world: World, amount: str, currency: str
) -> None:
...
def given_category_with_expense(
    world: World, name: str, amount: str, currency: str
) -> None:
...
def given_pre_upgrade_data(world: World) -> None:
    ...
    with engine.begin() as conn:
        ...
        conn.execute(sa_text(
            "INSERT INTO fx_rate (date, usd_cop) VALUES ('2026-05-10', 4000.0)"
        ))
...
def _view_last_expense(world: World) -> None:
    def action():
        ...
        world.viewed_cop_cents = _cop_equivalent_cents(
            world.session, tx.amount, tx.currency
        )
...
def when_daily_rate_update(world: World, value: str) -> None:
    def action():
        with mock.patch.object(
            daily_job, "fetch_usd_cop", return_value=Decimal(value)
        ):
            daily_job.run_daily(
                world.session, world.today, "https://fx.acceptance.invalid", None
            )
...
def _view_report(world: World, month: str) -> None:
    ...
    def action():
        world.report = reports.monthly_report(
            world.session, month, today=world.today
        )
...
def when_view_budget(world: World) -> None:
    def action():
        world.budget_lines = budgets.list_budgets(
            world.session, _month_str(world.today)
        )
...
def when_cross_currency_transfer(
    world: World, sent: str, sent_cur: str, src: str, received: str,
    recv_cur: str, dst: str,
) -> None:
    def action():
...
def when_same_currency_transfer(
    world: World, amount: str, currency: str, src: str, dst: str
) -> None:
    def action():
...
def when_upgrade_completes(world: World) -> None:
    ...
    if world.alembic_cfg is None:
        raise AssertionError(
            "scenario setup error: no old-design database was prepared"
        )
...
def when_view_expense_in_app(world: World) -> None:
    def action():
        ...
        resp = client.get(
            f"/api/transactions/{world.last_expense_id}", headers=auth
        )
        if resp.status_code != 200:
            raise AssertionError(
                f"GET /api/transactions/{world.last_expense_id} -> "
...
def when_assistant_lists_expense(world: World) -> None:
    def action():
        ...
        if world.last_expense_id is None:
            raise AssertionError("no expense was recorded earlier in the scenario")
        card = mcp_get_transaction(
            world.session, GetTransactionInput(tx_id=world.last_expense_id)
        )
        m = _re.search(r"\((" + _DEC + r") COP\)", card)
        if m is None:
            raise AssertionError(
                f"MCP transaction card shows no '(X COP)' equivalent: {card!r}"
            )
...
def then_expense_recorded(world: World, amount: str, currency: str) -> None:
    ...
    assert tx.amount == expected, (
        f"expected amount {expected} cents, got {tx.amount}"
    )
    assert tx.currency == currency, (
        f"expected currency {currency}, got {tx.currency}"
    )
...
def then_viewing_expense_no_rate(world: World) -> None:
    ...
    assert rate is None, (
        f"transaction still carries a frozen rate (fx_rate={rate}); "
        "ADR-0031 drops per-transaction rates"
    )
    assert converted is None, (
        f"transaction still carries a frozen converted amount "
        f"(to_base={converted}); ADR-0031 drops stored conversions"
    )
...
def then_account_balance(world: World, name: str, amount: str, currency: str) -> None:
    ...
    expected = major_to_cents(amount)
    assert acc.currency == currency, (
        f"account {name!r} is in {acc.currency}, expected {currency}"
    )
    ...
    assert acc.balance == expected, (
        f"account {name!r}: expected balance {expected} cents "
        f"({amount} {currency}), got {acc.balance}{context}"
    )
...
def then_cop_equivalent(world: World, amount: str) -> None:
    ...
    assert world.viewed_cop_cents == expected, (
        f"expected COP equivalent {expected} cents ({amount} COP), "
        f"got {world.viewed_cop_cents}"
    )
...
def _assert_report_total(world: World, month: str, amount: str) -> None:
    ...
    assert report.expense == expected, (
        f"month {month}: expected expense total {expected} cents "
        f"({amount} COP), got {report.expense}"
    )
...
def then_expense_cop_equivalent_now(world: World, amount: str) -> None:
    ...
    expected = major_to_cents(amount)
    assert got == expected, (
        f"expected COP equivalent {expected} cents ({amount} COP), got {got}"
    )


@step(
    r'the spending for "(?P<name>[^"]+)" shows (?P<amount>' + _DEC + r") COP"
)
def then_budget_spending(world: World, name: str, amount: str) -> None:
    ...
    line = next(
        (l for l in world.budget_lines if l.category_name == name), None
    )
    assert line is not None, (
        f"no budget line for category {name!r}; got "
        f"{[l.category_name for l in world.budget_lines]}"
    )
    expected = major_to_cents(amount)
    assert line.spent == expected, (
        f"category {name!r}: expected spending {expected} cents "
        f"({amount} COP), got {line.spent}"
    )
...
def then_current_trm(world: World, value: str) -> None:
    ...
    assert current == Decimal(value), (
        f"expected current TRM {value}, got {current}"
    )
...
def then_implied_rate(world: World, value: str) -> None:
    ...
    assert world.implied_rate == Decimal(value), (
        f"expected implied rate {value}, got {world.implied_rate}"
    )
...
def then_report_not_shown(world: World) -> None:
    assert world.report is None, (
        "the report was shown although no TRM is set (AC-9: reads must "
        "fail loud, never assume rate 1)"
    )
    world.consume_rejection(
        (MissingRate,), "viewing the report without a TRM"
    )
...
def then_told_to_set_trm(world: World) -> None:
    ...
    assert "trm" in str(err).lower(), (
        f"the error does not tell the user to set the TRM: {err}"
    )
...
def then_migration_preserves_amounts(world: World) -> None:
    with world.engine.connect() as conn:
        rows = conn.execute(sa_text(
            'SELECT payee, amount, currency FROM "transaction" ORDER BY id'
        )).fetchall()
    actual = [(r[0], r[1], r[2]) for r in rows]
    assert actual == world.migration_expected, (
        f"original amounts/currencies changed: expected "
        f"{world.migration_expected}, got {actual}"
    )
...
def then_viewing_any_transaction_no_rate(world: World) -> None:
    columns = {
        col["name"] for col in sa_inspect(world.engine).get_columns("transaction")
    }
...
def then_both_surfaces_agree(world: World, amount: str) -> None:
    ...
    assert world.app_cop_cents == expected_cents, (
        f"app (REST): expected {expected_cents} COP cents ({amount} COP), "
        f"got {world.app_cop_cents}"
    )
    assert world.mcp_cop_major == expected_major, (
        f"assistant (MCP): expected {amount} COP, got {world.mcp_cop_major}"
    )
