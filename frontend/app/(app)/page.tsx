    ...
    <div className="space-y-8">
      ...
      <div className="hero-glow animate-fade-up space-y-1">
        ...
        {sts.isLoading ? (
          ...
          <p className="text-sm" style={{ color: "var(--expense)" }}>No disponible</p>
        )}
      </div>
          ...
          <Card label="Ingresos · Gastos · Neto">
            {report.isLoading ? (
              ...
              <div className="space-y-2.5">
                <Row label="Ingresos">
                  <MoneyAmount cents={report.data.income} currency="COP" type="income" className="text-sm font-medium" />
                </Row>
                <Row label="Gastos">
                  <MoneyAmount cents={report.data.expense} currency="COP" type="expense" className="text-sm font-medium" />
                </Row>
                <hr style={{ borderColor: "var(--border)" }} />
                <Row label="Neto">
        ...
        <div className="animate-fade-up" style={{ animationDelay: "110ms" }}>
          <Card label="Saldos">
            {accounts.isLoading ? (
              ...
              <p className="text-sm" style={{ color: "var(--muted-foreground)" }}>Sin cuentas</p>
            )}
          </Card>
        </div>
        ...
        <div className="animate-fade-up" style={{ animationDelay: "130ms" }}>
          <Card label="Metas">
            {goals.isLoading ? (
              ...
              <p className="text-sm" style={{ color: "var(--muted-foreground)" }}>Sin metas activas</p>
            )}
          </Card>
        </div>
                  ...
                  <div className="space-y-2.5">
                    {over.map((e) => (
                      <Row key={e.category} label={e.category}>
                        <MoneyAmount cents={Math.abs(e.available)} currency="COP" type="expense" className="text-sm font-medium" />
                      </Row>
                    ))}
                  </div>
                ) : (
                  <p className="text-sm" style={{ color: "var(--income)" }}>Todos los sobres al día</p>
                );
              })()
            ) : (
              <p className="text-sm" style={{ color: "var(--muted-foreground)" }}>Sin datos</p>
