...
def test_missing_rate_uses_canonical_sentence():
    ...
    assert "No tengo la tasa USD→COP" in text
    assert "fijar_tasa_fx" in text


def test_not_found_passes_message_through():
    assert format.domain_error_text(NotFound("No encontré la cuenta 'X'.")) == (
        "No encontré la cuenta 'X'."
    )


def test_validation_error_is_framed_in_spanish():
...
def test_transfer_imbalance_is_framed():
    ...
    ).startswith("No pude registrar la transferencia:")
...
def test_expense_confirmation_cop_omits_equivalent():
    ...
    assert "Equivalente" not in text  # COP needs no to_base line
...
def test_expense_confirmation_usd_shows_to_base():
    ...
    assert "Equivalente: 49800.00 COP" in text
...
def test_fx_set_and_current():
    ...
    assert format.fx_current(Decimal("4150"), date(2026, 6, 18)) == (
        "Tasa vigente USD→COP al 2026-06-18: 4150"
    )
...
def test_accounts_table_and_empty():
    ...
    assert "Bancolombia" in table and "100000.00" in table and "| Cuenta |" in table
    assert format.accounts_table([]) == "No hay cuentas."
...
def test_categories_table_resolves_group_name():
    ...
    assert "Sueldo" in table and "sí" in table  # is_income
...
def test_tags_list():
    ...
    assert format.tags_list([]) == "No hay etiquetas."
...
def test_transactions_table_has_total_and_empty():
    ...
    assert "| Fecha |" in table
    assert "Total (COP): 50000.00" in table
    assert "2 transacción(es)" in table
    assert format.transactions_table([]) == "No hay transacciones para esos filtros."
