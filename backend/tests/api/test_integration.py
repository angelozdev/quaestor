def test_login_cookie_authorizes_same_endpoints_as_bearer(client):
    ...
    acc = client.post("/api/accounts", json={"name": "Cash", "type": "cash", "currency": "COP"})
...
def test_bearer_full_core_crud(client, auth):
    # accounts
    cash = client.post("/api/accounts", headers=auth, json={"name": "Cash", "type": "cash", "currency": "COP"}).json()
    bank = client.post("/api/accounts", headers=auth, json={"name": "Bank", "type": "debit", "currency": "COP"}).json()

    # category group + category
    group = client.post("/api/category-groups", headers=auth, json={"name": "Essentials"}).json()
    cat = client.post("/api/categories", headers=auth, json={"name": "Food", "group_id": group["id"]}).json()

    # tag
    tag = client.post("/api/tags", headers=auth, json={"name": "groceries"}).json()

    # fx override then expense in COP
    assert client.post("/api/fx", headers=auth, json={"date": "2026-06-17", "usd_cop": "4000"}).status_code == 201
    ...
    # settings
    assert (
        client.patch("/api/settings", headers=auth, json={"default_source_account_id": bank["id"]}).status_code == 200
    )
