...
**Fuera:**
- Presupuestos y metas (P4). P3 solo deja **registrado el seam de hooks** para que P4 enganche `aplicar_aportes_meta`.
...
- `por_pagar(desde, hasta) -> {items: list[Transaction], total_base: int}` — todas las tx `planned` con vencimiento en la ventana `[desde, hasta]`, ordenadas por fecha, + total en `to_base` COP.
...
El rollover también debe **aplicar aportes de metas**, pero **las metas las define P4** y el orden de build es **P3 → P4**. Para no acoplar P3 a un modelo que aún no existe, `cerrar_mes` se diseña como **lista extensible de hooks**:

- **P3** registra `aplicar_recurrentes` (este sub-proyecto).
- **P4**, cuando exista, registrará `aplicar_aportes_meta` vía `registrar_hook_rollover(...)` **sin modificar `cerrar_mes`**. Ese hook creará `GoalContribution` + transfer por cada `Goal` activa, con su propia idempotencia (P4 la define, p. ej. una contribución por `(goal_id, period)`).
...
**Listo cuando** todos los tests anteriores pasan en verde, el constraint único `(recurring_id, period)` está aplicado, y existen las **tools MCP** (`crear_recurrente`, `listar_recurrentes`, `planear_pago`, `confirmar_pago`, `omitir_pago`, `omitir_recurrente`, `por_pagar`, `cerrar_mes`) y los **endpoints REST** correspondientes — el **wire concreto** lo hacen P2 (`/recurring`, `/planned`, `/rollover`) y P1 sobre estos services, sin lógica duplicada.
...
- **P2 (MCP):** expone una tool por service (mismos verbos en lenguaje natural: *"¿qué me falta por pagar esta semana?"* → `por_pagar`; *"cierra junio"* → `cerrar_mes`).
- **P4 (Presupuestos + Metas):** se engancha al **hook de rollover** con `aplicar_aportes_meta` sin tocar `cerrar_mes` (seam descrito arriba). Orden de build P3 → P4.
