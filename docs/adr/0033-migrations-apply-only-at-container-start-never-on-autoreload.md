...
Chosen option: **5 — remove the reload trigger by not bind-mounting `src` in the
pg profile, plus a pre-flight check that refuses to start `dev-prod` while
revisions are pending.**

Option 1 deletes the mechanism behind both incidents: with no bind mount there
is no WatchFiles event, so trigger 2 can never fire against production. It is a
compose-only change, contradicts no ADR, and losing hot-reload against the
production database is a feature, not a cost — code should be edited against the
sqlite sandbox. Option 3 then covers what remains: trigger 1 is legitimate and
human-initiated, but it should announce a pending schema change and demand a
backup rather than apply it silently. Together they turn "a file write migrates
production" into "a human sees `2 pending revisions — run just backup, then just
migrate`".
...
- Bad / cost: editing backend code in the pg profile needs a rebuild.
...
- Open: whether `dev-real` (Render standby) deserves the same treatment. It is
  frozen per ADR-0030, but it carries the same bind mount today.

## Confirmation

- A test asserting the pg profile's api service declares no `./backend/src`
  volume (parse `docker-compose.yml`; the sqlite profile keeps it).
